---
title: "Optional self-hosted cloud sync"
...
prompt text to the sync hub you configure. Don't enable it if
...
When enabled, the worker also holds one WebSocket to the hub and
...
1. A sync token and user id issued by your own compatible hub.
2. A sync hub URL in `CLAUDE_MEM_CLOUD_SYNC_HUB_URL`. The included
   `workers/sync-hub/` source can be deployed to infrastructure you control.
...
| Key | Default | Meaning |
|-----|---------|---------|
| `CLAUDE_MEM_CLOUD_SYNC_TOKEN` | `''` | Token issued by your configured sync hub. Sent as `Authorization: Bearer`. |
| `CLAUDE_MEM_CLOUD_SYNC_USER_ID` | `''` | User id issued by your configured sync hub. |
