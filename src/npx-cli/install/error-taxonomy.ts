...
export const ERROR_CATEGORIES: ErrorCategory[] = [
  ...
  },
  {
    ...
    remediation: () =>
      'ERESOLVE peer-dependency conflict in marketplace deps that --legacy-peer-deps could not resolve. Open an issue at https://github.com/gaalbu/claude-mem/issues with the conflicting peer ranges shown above.',
  },
  {
  ...
  },
  {
    ...
    remediation: () =>
      'Existing plugin.json is corrupt. Run `rm -rf ~/.claude/plugins/marketplaces/gaalbu` and re-run `npx claude-mem install`.',
  },
  {
    id: 'all-ides-failed',
    severity: ErrorSeverity.ABORT,
  ...
  },
  {
    ...
    match: () => true,
    remediation: (ctx) =>
      `An unexpected installer error occurred. Capture ${ctx.dataDir}/last-install-error.json and open an issue at https://github.com/gaalbu/claude-mem/issues.`,
  },
];
