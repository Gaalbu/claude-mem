...
function registerMarketplace(): void {
  ...
  knownMarketplaces['gaalbu'] = {
    source: {
      source: 'github',
      repo: 'gaalbu/claude-mem',
    },
...
function registerPlugin(version: string): void {
  ...
  installedPlugins.plugins['claude-mem@gaalbu'] = [
    {
      scope: 'user',
...
function enablePluginInClaudeSettings(): void {
  ...
  settings.enabledPlugins['claude-mem@gaalbu'] = true;
...
async function promptTelemetryOptIn(): Promise<void> {
  ...
  const consent = await p.confirm({
    message: 'Share anonymized usage data with CMEM? It is off by default and can be enabled if you want to help improve the product.',
    initialValue: false,
  });
