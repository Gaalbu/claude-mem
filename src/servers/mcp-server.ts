...
function detectMissingMarketplaceMarker(): void {
  ...
  const marketplaceCandidates = [
    resolve(home, '.claude', 'plugins', 'marketplaces', 'gaalbu'),
    resolve(home, '.config', 'claude', 'plugins', 'marketplaces', 'gaalbu'),
  ];
  const present = marketplaceCandidates.some(p => p && existsSync(p));
  const cacheCandidates = [
    resolve(home, '.claude', 'plugins', 'cache', 'gaalbu', 'claude-mem'),
    resolve(home, '.config', 'claude', 'plugins', 'cache', 'gaalbu', 'claude-mem'),
  ];
