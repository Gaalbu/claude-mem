 ...
 *   4. $_C/plugins/marketplaces/gaalbu/plugin (marketplace install)
 */
...
function candidateBlock(options: ShellTemplateOptions): string {
  ...
  const allGlobs = [...extraCacheRoots, '$_C/plugins/cache/gaalbu/claude-mem']
    .map((root) => `"${root}"/[0-9]*/`)
    .join(' ');
...
function buildMcpNodeLauncher(options: ShellTemplateOptions): string {
  ...
  const cacheRoots = [
    ...
    '$_C/plugins/cache/gaalbu/claude-mem',
  ].map(shTokenToNode);
  const marketplace = shTokenToNode('$_C/plugins/marketplaces/gaalbu/plugin');
...
export function buildCodexWindowsCommand(
  ...
  const parts = [
    ...
    "const cache=p.join(C,'plugins','cache','gaalbu','claude-mem');",
    // S/W mirror compareVersionsDescending in src/shared/worker-utils.ts and
    ...
    "roots.push(p.join(C,'plugins','marketplaces','gaalbu','plugin'));",
    "let R=null;",
